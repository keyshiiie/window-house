// src/components/services/data/servicesData.ts
import type { Services } from '../types/Services';

export const servicesData: Services[] = [
    {
        id: 1,
        title: 'Установка пластиковых окон',
        description: 'Устанавливаем пластиковые окна Rehau от проверенного производителя с гарантией качества',
        icon: '/icons/services/01.svg',
        backgroundImage: '/images/service-1-bg.jpg',
        fullDescription: 'Профессиональное остекление балконов и лоджий любой сложности. Используем современные профильные системы Rehau, KBE, Proplex. Возможность выноса остекления за перила для увеличения полезной площади.',
        price: 'от 25 000 ₽',
        images: [
            '/images/services-slider-mock-1.jpg',
            '/images/services-slider-mock-1.jpg',
            '/images/services-slider-mock-1.jpg',
        ],
        features: [
            {
                id: 1,
                title: 'Пластиковые окна',
                description: 'Использование ламинационных окон, ламинации в массе, 3д-ламинации, а также покраска окон.',
                icon: '/icons/services-details/01.svg'
            },
            {
                id: 2,
                title: 'Подоконники',
                description: 'Качественный подоконник не царапается, выдерживает большие нагрузки и будет служить вам долгие годы. ',
                icon: '/icons/services-details/01.svg'
            },
            {
                id: 3,
                title: 'Проветриватели',
                description: 'Постоянное обновление воздуха в комнате снижает риск аллергии, улучшает сон и благоприятно влияет на самочувствие. ',
                icon: '/icons/services-details/02.svg'
            },
            {
                id: 4,
                title: 'Ламинация',
                description: 'Пластиковые окна могут быть любого нужного вам цвета. Внутри и снаружи. ',
                icon: '/icons/services-details/02.svg'
            },
            {
                id: 5,
                title: 'Цветные ручки',
                description: 'Оригинальные оконные ручки подчеркнут дизайн окна. Подберем оптимальный цвет ручек в зависимости от цвета всей конструкции.',
                icon: '/icons/services-details/05.svg'
            },{
                id: 6,
                title: 'Москитные сетки',
                description: 'Нежеланные насекомые останутся на улице, а ваша кошка - в квартире. Устанавливаем сетки для пластиковых окон любых форм и размеров.',
                icon: '/icons/services-details/06.svg'
            }
        ]
    },
    {
        id: 2,
        title: 'Остекление балконов и лоджий',
        description: 'Занимаемся остеклением и отделкой балконов и лоджий. Теплое и холодное остекление, изготовление выносов и крыш. Беремся за сложные объекты',
        icon: '/icons/services/02.svg',
        backgroundImage: '/images/service-2-bg.jpg',
        fullDescription: 'Профессиональное остекление балконов и лоджий любой сложности. Используем современные профильные системы Rehau, KBE, Proplex. Возможность выноса остекления за перила для увеличения полезной площади.',
        price: 'от 25 000 ₽',
        images: [
            '/images/services-slider-mock-2.jpg',
            '/images/services-slider-mock-2.jpg',
            '/images/services-slider-mock-2.jpg',
        ],
        features: [
            {
                id: 1,
                title: 'Пластиковые окна',
                description: 'Использование ламинационных окон, ламинации в массе, 3д-ламинации, а также покраска окон.',
                icon: '/icons/services-details/01.svg'
            },
            {
                id: 2,
                title: 'Подоконники',
                description: 'Качественный подоконник не царапается, выдерживает большие нагрузки и будет служить вам долгие годы. ',
                icon: '/icons/services-details/01.svg'
            },
            {
                id: 3,
                title: 'Проветриватели',
                description: 'Постоянное обновление воздуха в комнате снижает риск аллергии, улучшает сон и благоприятно влияет на самочувствие. ',
                icon: '/icons/services-details/02.svg'
            },
            {
                id: 4,
                title: 'Ламинация',
                description: 'Пластиковые окна могут быть любого нужного вам цвета. Внутри и снаружи. ',
                icon: '/icons/services-details/02.svg'
            },
            {
                id: 5,
                title: 'Цветные ручки',
                description: 'Оригинальные оконные ручки подчеркнут дизайн окна. Подберем оптимальный цвет ручек в зависимости от цвета всей конструкции.',
                icon: '/icons/services-details/05.svg'
            },{
                id: 6,
                title: 'Москитные сетки',
                description: 'Нежеланные насекомые останутся на улице, а ваша кошка - в квартире. Устанавливаем сетки для пластиковых окон любых форм и размеров.',
                icon: '/icons/services-details/06.svg'
            }
        ]
    },
    {
        id: 3,
        title: 'Остекление коттеджей и дач',
        description: 'Остекление и отделка под ключ: от коттеджей и дач до террас и беседок. Выделяем личного технолога под каждый объект',
        icon: '/icons/services/03.svg',
        backgroundImage: '/images/service-3-bg.jpg',
        fullDescription: 'Профессиональное остекление балконов и лоджий любой сложности. Используем современные профильные системы Rehau, KBE, Proplex. Возможность выноса остекления за перила для увеличения полезной площади.',
        price: 'от 25 000 ₽',
        images: [
            '/images/services-slider-mock-3.jpg',
            '/images/services-slider-mock-3.jpg',
            '/images/services-slider-mock-3.jpg',
        ],
        features: [
            {
                id: 1,
                title: 'Пластиковые окна',
                description: 'Использование ламинационных окон, ламинации в массе, 3д-ламинации, а также покраска окон.',
                icon: '/icons/services-details/01.svg'
            },
            {
                id: 2,
                title: 'Подоконники',
                description: 'Качественный подоконник не царапается, выдерживает большие нагрузки и будет служить вам долгие годы. ',
                icon: '/icons/services-details/01.svg'
            },
            {
                id: 3,
                title: 'Проветриватели',
                description: 'Постоянное обновление воздуха в комнате снижает риск аллергии, улучшает сон и благоприятно влияет на самочувствие. ',
                icon: '/icons/services-details/02.svg'
            },
            {
                id: 4,
                title: 'Ламинация',
                description: 'Пластиковые окна могут быть любого нужного вам цвета. Внутри и снаружи. ',
                icon: '/icons/services-details/02.svg'
            },
            {
                id: 5,
                title: 'Цветные ручки',
                description: 'Оригинальные оконные ручки подчеркнут дизайн окна. Подберем оптимальный цвет ручек в зависимости от цвета всей конструкции.',
                icon: '/icons/services-details/05.svg'
            },{
                id: 6,
                title: 'Москитные сетки',
                description: 'Нежеланные насекомые останутся на улице, а ваша кошка - в квартире. Устанавливаем сетки для пластиковых окон любых форм и размеров.',
                icon: '/icons/services-details/06.svg'
            }
        ]
    },
    {
        id: 4,
        title: 'Остекление корпоративных объектов',
        description: 'Реализуем сложные проекты по остеклению торговых центров, ресторанов, производственных помещений и офисов',
        icon: '/icons/services/04.svg',
        backgroundImage: '/images/service-4-bg.jpg',
        fullDescription: 'Профессиональное остекление балконов и лоджий любой сложности. Используем современные профильные системы Rehau, KBE, Proplex. Возможность выноса остекления за перила для увеличения полезной площади.',
        price: 'от 25 000 ₽',
        images: [
            '/images/services-slider-mock-4.jpg',
            '/images/services-slider-mock-4.jpg',
            '/images/services-slider-mock-4.jpg',
        ],
        features: [
            {
                id: 1,
                title: 'Пластиковые окна',
                description: 'Использование ламинационных окон, ламинации в массе, 3д-ламинации, а также покраска окон.',
                icon: '/icons/services-details/01.svg'
            },
            {
                id: 2,
                title: 'Подоконники',
                description: 'Качественный подоконник не царапается, выдерживает большие нагрузки и будет служить вам долгие годы. ',
                icon: '/icons/services-details/01.svg'
            },
            {
                id: 3,
                title: 'Проветриватели',
                description: 'Постоянное обновление воздуха в комнате снижает риск аллергии, улучшает сон и благоприятно влияет на самочувствие. ',
                icon: '/icons/services-details/02.svg'
            },
            {
                id: 4,
                title: 'Ламинация',
                description: 'Пластиковые окна могут быть любого нужного вам цвета. Внутри и снаружи. ',
                icon: '/icons/services-details/02.svg'
            },
            {
                id: 5,
                title: 'Цветные ручки',
                description: 'Оригинальные оконные ручки подчеркнут дизайн окна. Подберем оптимальный цвет ручек в зависимости от цвета всей конструкции.',
                icon: '/icons/services-details/05.svg'
            },{
                id: 6,
                title: 'Москитные сетки',
                description: 'Нежеланные насекомые останутся на улице, а ваша кошка - в квартире. Устанавливаем сетки для пластиковых окон любых форм и размеров.',
                icon: '/icons/services-details/06.svg'
            }
        ]
    },
];